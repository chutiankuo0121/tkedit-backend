...
class Video_segment(Visual_segment):
    ...
    def add_animation(self, animation_type: Union[Intro_type, Outro_type, Group_animation_type],
        ...
        # [修复] 确保动画容器的ID总是在引用列表中
        if self.animations_instance.animation_id not in self.extra_material_refs:
    ...
    def export_json(self) -> Dict[str, Any]:
        json_obj = super().export_json()
        json_obj.update({
            "material_id": self.material_id,
            "type": "video",
            "volume": self.volume,
            "material_name": self.material_instance.material_name,
            "is_placeholder": False,
        })
        # [修复] 将所有引用（动画、滤镜等）统一导出
        all_refs = []
        if self.animations_instance:
            all_refs.append(self.animations_instance.animation_id)
        
        all_refs.extend([f.global_id for f in self.filters])
        all_refs.extend([e.global_id for e in self.effects])
        if self.mask:
            all_refs.append(self.mask.global_id)
        if self.transition:
            all_refs.append(self.transition.global_id)
        if self.background_filling:
            all_refs.append(self.background_filling.global_id)

        if all_refs:
            json_obj["extra_material_refs"] = all_refs
